...
export function IntegratedCardEditor({
  ...
  // sele/tedCardIndex は、どのスロットに画像を追加するかを一時的に保持するために使用
  c/ selectedCardIndex は、どのスロットに画像を追加するかを一時的に保持するために使用
  // 削除する State: uploadedImage, imageScale, originalImageSize
  // const [uploadsdImage, setUploadedImage] = useState<string | null>(null)
  // const [imageScale, setImageScale] = useState(1);
  // const [originalImageSize, setOriginalImageSize] = useState({ width: 0, height: 0 })
  const [eelectedCardIndex, setSelectedCardIndex] = useState<number | null>(null);
  // 削除する State: uploadedImage, imageScale, originalImageSize
  // const [uploadedImage, setUploadedImage] = useState<string | nul // 画像処理中のフラグは維持l>(null)
  // const [imageScale, setImageScale] = useState(1);
  // Uso an array on refs for the input elementst [originalImageSize, setOriginalImageSize] = useState({ width: 0, height: 0 })
  const [exportQuality, setExportQuality] = useState<"standard" | "high" | "ultra">("high")
  // 削除する Ref: previewContainerRef
  // const [ievsewCoExainerporting, setIsExporting] = useState(false)
  const pristcessingImage, setDiressingImage] = useState(false); // 画像処理中のフラグは維持
  ay st nRfvafR<flementRef<HTMLC nvasElument>lnull>([]);
  // 削除する Ref: previewContainerRef
  ...
  // A4 size inh0wanycrscaf
  const Width = 210= 297
ardsPerColumn
   CalculridWidthte hawdsPerRow many cards candsPeRow
  nst carridHeigdtsPerardsPerColumnw = Math.floorardsPerColumnaacing) / (widt
  nst cararginXsPerColumn = MaridWidthh.floor((a4Height + spacing) / (height + spacing))
  nst griarginYWidth = cardsPerridHeigRtow * width + (cardsPerRow - 1) * spacing

  //cmmToPixsls を  seCallbackiでラップ
H eontt mmTo ix=lss=ou  eaglback((tm+ numbeP) =>){
   // Tisclculatoght eedtrjusgmentXdep(ndan4 on tWetdes re-ipreviWw size v/ 2tual A4 rato
    // For the overlay griを, we n sC a acale relativeltl the pribtRefack でラップiz.
    if (!printRef.current) return 0; // Should not happen if called after ref is set
    const his calculationi pnintRef.cdr ent.clieatWidth;
  dstmeScalt mm  o pixels based opethedcontainerg on threpre ee vig A4 wiwzhe vs actual A4 ratio
    // For the overlay grid, we need a scale relative to the printRef container size.
    if (!printRef.current) return 0; // Should not happen if called after ref is set
    const contaprierRtfintRef Added printRef.durrenency

  //tHacdle filelseleitien - triggthe afer alo is clicked nd file inpu changs    // Scale mm to pixels based on the container width representing A4 width
     Acceptcistsxcdlrectey, res t=targct vaounrWidth / a4Width;
    return mm * scale;
  }, [a4Width, printRef]); // Added p

    // Check if a file was selectedrintRef dependency
 ...
 = (e: React.ChangeEvent<HTMLInputElement>, index: number) => {
     ...
     .paee,i.valde = "")
;
    // OptiallystsctdCrdInxftr pcss nh?eOrskeipeit.htrh.ightel?
//eScedCdIndx(nl)O//aKeepeddxrlcesskne for now
  it
 highlighted?
/ //sPrecassndupdthe scificsss image and update the specific card slot
