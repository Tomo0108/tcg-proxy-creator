...
export function IntegratedCardEditor({
  ...
  // Modified to accept index directly
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>, index: number) => {
    const file = e.target.files?.[0];
    // Check if a file was selected
    if (file) {
      // Use the index passed directly from the onChange handler
                  ...
                  >
                    ...
                    {Array(cardsPerRow * cardsPerColumn).fill(0).map((_, index) => (
                      <div
                        ...
                        onClick={() => {
                          setSelectedCardIndex(index); // Set the index to know where to put the image
                          fileInputRef.current?.click(); // Trigger file input
                        }}
                      >
                        {/* Hidden file input triggered by cell click - Pass index to handler */}
                        <Input
                          ref={fileInputRef} // ref is still needed to trigger click()
                          type="file"
                          accept="image/*"
                          className="hidden"
                          onChange={(e) => handleFileChange(e, index)} // Pass index here
                        />
