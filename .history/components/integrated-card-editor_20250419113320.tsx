...
export function IntegratedCardEditor({
  ...
  const handlePointerDown = (index: number) => {
    ...
    }, LONG_PRESS_DURATION); // 修正: setTimeout の閉じ括弧
  };
  ...
  const processImage = useCallback((file: File, indices: number[]) => {
    ...
    reader.onerror = () => {
      setIsProcessingImage(false);
      setSelectedCardIndices([]);
      toast({ title: "ファイル読み込みエラー", description: "ファイルの読み込みに失敗しました。", variant: "destructive" }); // 修正: カンマ追加
    };
    reader.readAsDataURL(file);
  }, [onCardUpdate, cardType, t, cardsPerRow, cardsPerColumn]); // 修正: 依存配列の閉じ括弧
  ...
  const renderCanvas = useCallback(() => {
    ...
    const loadPromises = cards.slice(0, cardsPerRow * cardsPerColumn).map((card, index) => {
      ...
      // 修正: 変数宣言を map の外から参照するように変更 (map 内の再宣言を削除)
      ...
      return new Promise<void>((resolve) => {
        ...
        img.onload = () => {
          const drawX = drawXBg; // map の外の変数を使用
          const drawY = drawYBg; // map の外の変数を使用
          const drawCardWidth = drawCardWidthBg; // map の外の変数を使用
          const drawCardHeight = drawCardHeightBg; // map の外の変数を使用
  ...
  }, [cards, spacing, cardType, a4Width, a4Height, cardWidthMM, cardHeightMM, marginXMM, marginYMM, cardsPerRow, cardsPerColumn, mmToPixels, containerWidth]); // 修正: 依存配列の閉じ括弧
  ...
  useEffect(() => {
    ...
    const observer = new ResizeObserver(entries => {
        ...
        animationFrameId = requestAnimationFrame(() => { // 修正: コールバックの閉じ括弧
        ...
        }); // 修正: requestAnimationFrame の閉じ括弧
    });
    observer.observe(element, { box: 'content-box' }); // 修正: observe の引数カンマ
    return () => {
      if (animationFrameId) { cancelAnimationFrame(animationFrameId); }
                      ...
                      {Array(cardsPerRow * cardsPerColumn).fill(0).map((_, index) => (
                        ...
                        >
                          <Input
                            ...
                            type="file" accept="image/*" className="hidden"
                            onChange={(e) => handleFileChange(e, index)} // 修正: onChange の構文
                          />
                          {cards[index] && (
                            <Button
