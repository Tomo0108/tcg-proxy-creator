...
export function IntegratedCardEditor({
  ...
  const handlePointerLeave = () => {
    ...
    // isLongPressing.current = false; // 離れたら長押し状態も解除するならコメント解除
  };
  // --- 長押し・タップ処理ここまで ---
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>, index: number) => {
    ...
    if (file) {
      ...
      if (index === -1 && selectedCardIndices.length > 0) {
        ...
        // handlePointerDownで選択状態は更新されているはずなので、そのまま処理
        console.log(`Processing image for single index: ${index}`);
        processImage(file, [index]); // 単一要素の配列として渡す
      }
    }
    e.target.value = "";
  };
  ...
  const processImage = useCallback((file: File, indices: number[]) => {
    ...
    reader.onload = (event) => {
      ...
      img.onload = () => {
        ...
        indices.forEach(idx => {
          // 有効なインデックスか確認 (cardsPerRow, cardsPerColumn が計算済みか)
  ...
  }, [onCardUpdate, cardType, t, cardsPerRow, cardsPerColumn]); // 修正: 依存関係
