...
export function IntegratedCardEditor({
  ...
  const renderCanvas = () => {
    ...
    // Set canvas drawing buffer size to match its display size from parent
    // This prevents the canvas element itself from overflowing its container
    canvas.width = printRef.current.clientWidth;
    canvas.height = printRef.current.clientHeight;
