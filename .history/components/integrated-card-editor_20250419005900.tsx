...
export function IntegratedCardEditor({
  ...
  const renderCanvas = useCallback(() => {
    ...
    // --- Debug Log Start ---
    console.log(`[Debug] printRef size: ${displayWidth}x${displayHeight} (Aspect Ratio: ${displayWidth / displayHeight})`);
    console.log(`[Debug] Expected A4 Aspect Ratio: ${a4Width / a4Height}`);
    const cardWidthPx = mmToPixels(width);
    const cardHeightPx = mmToPixels(height);
    console.log(`[Debug] Card dimensions (mm): ${width}x${height}`);
    console.log(`[Debug] Calculated card size (px): ${cardWidthPx.toFixed(2)}x${cardHeightPx.toFixed(2)}`);
    console.log(`[Debug] Calculated card aspect ratio (px): ${(cardWidthPx / cardHeightPx).toFixed(4)}`);
    console.log(`[Debug] Expected card aspect ratio (mm): ${(width / height).toFixed(4)}`);
    // --- Debug Log End ---
