...
export function IntegratedCardEditor({
  ...
  const renderCanvas = () => {
    ...
    // Set canvas drawing buffer size to match its CSS display size
    ...
    // Clear canvas (important to do after setting size)
    ctx.fillStyle = "white"
    ...
    const loadImages = cards.map((card, index) => {
      ...
      // Remove calculation from here, it's now inside onload
      // const x = mmToPixels(marginX + col * (width + spacing))
      // const y = mmToPixels(marginY + row * (height + spacing))
      // const cardWidthPx = mmToPixels(width)
      // const cardHeightPx = mmToPixels(height)

      // Remove background fill from here, it's now inside onload or not needed
      ...
      if (card.image) {
        return new Promise<void>((resolve) => {
          ...
          img.onload = () => {
            // Calculate card position and size inside the callback using variables from renderCanvas scope
            // Ensure these variables (drawOffsetX, scaleFactor, etc.) are correctly referenced.
            ...
            const cardAspectRatio = drawCardWidth / drawCardHeight; // Use calculated values

            let targetWidth, targetHeight;
            if (imgAspectRatio > cardAspectRatio) { // Image is wider than card area needs
                targetWidth = drawCardWidth * card.scale; // Use card.scale directly
                targetHeight = targetWidth / imgAspectRatio;
            } else { // Image is taller than card area needs
                targetHeight = drawCardHeight * card.scale; // Use card.scale directly
                targetWidth = targetHeight * imgAspectRatio;
            }

            // Calculate position to center the scaled image within the card area
            const imgDrawX = drawX + (drawCardWidth - targetWidth) / 2; // Use calculated values
            const imgDrawY = drawY + (drawCardHeight - targetHeight) / 2; // Use calculated values
