    ...
    const loadImages = cards.map((card, index) => {
      ...
      if (card.image) {
        return new Promise<void>((resolve) => {
          ...
          img.onload = () => {
            // Recalculate card position and size inside the callback using scaleFactor and offsets
            ...
            if (imgAspectRatio > cardAspectRatio) { // Image is wider than card area needs
                targetWidth = drawCardWidth * card.scale; // Use recalculated value
                targetHeight = targetWidth / imgAspectRatio;
            } else { // Image is taller than card area needs
                targetHeight = drawCardHeight * card.scale; // Use recalculated value
