...
export function IntegratedCardEditor({
  ...
  const renderCanvas = () => {
    ...
    const loadImages = cards.map((card, index) => {
      ...
      const row = Math.floor(index / cardsPerRow) // Ensure row is defined here
      const col = index % cardsPerRow

      // Card position/size calculation moved inside onload

      // Draw card background (optional, white background is already there)
      ...
      if (card.image) {
        return new Promise<void>((resolve) => {
          ...
          img.onload = () => {
            ...
            // --- Start Clipping ---
            ctx.beginPath();
            ctx.rect(drawX, drawY, drawCardWidth, drawCardHeight); // Clipping path matching card area
            ctx.clip();
            // --- End Clipping ---
