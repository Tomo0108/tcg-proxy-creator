...
export function IntegratedCardEditor({
  ...
  useEffect(() => {
    if (uploadedImage && previewContainerRef.current && originalImageSize.width > 0 && originalImageSize.height > 0) {
      ...
      // Only update if the scale is significantly different to avoid infinite loops
      if (Math.abs(imageScale - newScale) > 0.001) {
      ...
      if (card.image) {
        return new Promise<void>((resolve) => {
          ...
          img.onload = () => {
            ...
            const scaleAdjustment = cardWidthPx / editorPreviewCardWidthPx;
