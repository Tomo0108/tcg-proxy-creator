...
export function IntegratedCardEditor({
  ...
  // Handle click for the dedicated upload area - Dynamically set 'multiple' attribute
  const handleUploadButtonClick = () => {
    let targetIndices: number[] | null = null;
    const maxSlots = cardsPerRow * cardsPerColumn;
    const isSlotSelected = selectedCardIndices.length > 0;

    if (isSlotSelected) {
      // Case 1: Slots are selected
      targetIndices = [...selectedCardIndices];
    } else {
      // Case 2: No slots selected (find empty slots)
      const emptyIndices: number[] = [];
      for (let i = 0; i < maxSlots && emptyIndices.length < 9; i++) { // Limit to 9 for multi-upload
    ...
    if (targetIndices && targetIndices.length > 0) {
      ...
      if (validTargetIndices.length > 0) {
        ...
        // Dynamically set the 'multiple' attribute on the file input
        if (uploadInputRef.current) {
          uploadInputRef.current.multiple = !isSlotSelected; // Allow multiple only if NO slots are selected
        }
        uploadInputRef.current?.click(); // Trigger file selection
      } else {
        ...
        <Card className="col-span-1 border-gold-500">
          <CardContent className="p-6">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-semibold">{t("layout.preview")}</h3> {/* Changed font-medium to font-semibold */}
