...
interface CardInstanceData {
  ...
  // opacity と isFadingOut はスケール制御で代替するため削除
}
...
const FADE_OUT_START_Y = -5; // フェードアウト開始Y座標
const RESET_Y = -7; // リセットY座標 (フェードアウト終了点より下)
...
export function CardInstances({ count, isMobile }: CardInstancesProps) {
  ...
  // マテリアル作成
  // 側面用のシンプルなマテリアル
  const sideMaterial = useMemo(() => new THREE.MeshBasicMaterial({ color: 0xffffff, transparent: true, opacity: 0.5, depthWrite: false, alphaTest: 0.1 }), []); // opacity: 0.5 を追加
  // 表面マテリアル (配列)
  const frontMaterials = useMemo(() =>
    frontTextures.map(tex => new THREE.MeshBasicMaterial({ map: tex, transparent: true, opacity: 0.5, depthWrite: false, alphaTest: 0.1 })) // opacity: 0.5 を追加
  , [frontTextures]);
  // 裏面マテリアル
  const backMaterial = useMemo(() =>
    // BoxGeometryの場合、裏面は自動的に反転されるため BackSide は不要かもしれないが、念のため付けておく
    new THREE.MeshBasicMaterial({ map: backTexture, side: THREE.FrontSide, transparent: true, opacity: 0.5, depthWrite: false, alphaTest: 0.1 }) // opacity: 0.5 を追加
  , [backTexture]);
