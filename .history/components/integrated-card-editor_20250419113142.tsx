...
export function IntegratedCardEditor({
  ...
  // const [selectedCardIndex, setSelectedCardIndex] = useState<number | null>(null); // 単一選択から複数選択へ変更
  ...
  const handlePointerDown = (index: number) => {
    isLongPressing.current = false; // 長押しフラグをリセット
    // 前回のタイマーが残っていればクリア
    ...
    longPressTimer.current = setTimeout(() => {
      isLongPressing.current = true; // 長押しと判定
      console.log(`Long press detected on index: ${index}`);
      // 複数選択されているか、または現在押下中のセルが選択されている場合
