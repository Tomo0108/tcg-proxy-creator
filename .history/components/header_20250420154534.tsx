      ...
      <div className="container flex h-16 items-center justify-between px-4 sm:px-6 lg:px-8">
        ...
        >
          <Image
            ...
            className="h-10 w-10"
          />
          TCG Proxy Creator {/* Replace translation with static text */}
        </Link>
        ...
        <div className="md:hidden">
          <Sheet open={isMenuOpen} onOpenChange={setIsMenuOpen}>
            <SheetTrigger asChild>
              {/* Add text-white to force icon color */}
                ...
                <nav className="grid gap-2">
                  {navItems.map((item) => (
                      <Link
                        ...
                        href={item.href}
                        className={cn(
                          "flex items-center gap-3 rounded-md px-3 py-2 text-base font-medium text-foreground", // Add text-foreground
                          pathname === item.href
                            ? "bg-accent" // Keep bg-accent for active, text-foreground handles text color
                            : "hover:bg-accent" // Keep hover:bg-accent, text-foreground handles text color
                        )}
