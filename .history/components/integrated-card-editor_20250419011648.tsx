...
export function IntegratedCardEditor({
  ...
  const [imageScale, setImageScale] = useState(1); // User-controlled scale relative to fit (1 = fitted)
    ...
    reader.onload = (event) => {
      ...
      img.onload = () => {
        ...
        if (previewContainerRef.current && newOriginalSize.width > 0 && newOriginalSize.height > 0) {
          ...
          setImageScale(fitScale); // Set initial scale to fit
          console.log("Calculated initial fit scale:", fitScale);
        } else {
          setImageScale(1); // Fallback if container ref or image size is invalid
        }
    ...
    const loadImages = cards.map((card, index) => {
      ...
      return new Promise<void>((resolve) => {
        ...
        img.onload = () => {
          ...
          const cardScale = card.scale || 1; // Use saved scale (relative to original image size)

          // Calculate the target size based on original image size and saved scale
          // This ensures the scale from the editor preview is applied correctly
          const targetWidth = img.width * cardScale;
          const targetHeight = img.height * cardScale;

          // Center the scaled image within the card area (drawX, drawY, drawCardWidth, drawCardHeight)
          // The image is drawn centered within the clipped card slot boundaries.
